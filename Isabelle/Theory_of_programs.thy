...
theorem cond_for_commutative_1: "Range_p p\<^sub>1 \<inter> Pre p\<^sub>2 = {} \<Longrightarrow> Range_p p\<^sub>2 \<inter> Pre p\<^sub>1 = {} \<Longrightarrow> commute_programs p\<^sub>1 p\<^sub>2"
  by (auto simp: used_states_def commute_programs_def composition_def equiv_def corestrict_r_def Field_def restr_post_def restrict_r_def Range_p_def Range_iff Int_def)
...
theorem "used_states p\<^sub>1 \<inter> used_states p\<^sub>2 = {} \<Longrightarrow> commute_programs p\<^sub>1 p\<^sub>2"
