...
theorem NOT_corestricted_p_refines_p: "p \<setminus>\<^sub>p C \<sqsubseteq>\<^sub>p p"
  oops \<comment> \<open>Not true as corestriction might remove a post relation completely\<close>

theorem NOT_p_refines_corestricted_p: "p \<sqsubseteq>\<^sub>p p \<setminus>\<^sub>p C"
  oops \<comment> \<open>Not true as corestriction might remove a post ambiguity\<close>

theorem corestricted_refines_unrestricted_1: "p \<setminus>\<^sub>p C \<sqsubseteq>\<^sub>p p" \<comment> \<open>T31\<close>
  oops
theorem unrestricted_refines_corestricted_1: "p \<sqsubseteq>\<^sub>p p \<setminus>\<^sub>p C"
  oops
theorem corestricted_refines_unrestricted_2: "is_feasible p \<Longrightarrow> p \<setminus>\<^sub>p C \<sqsubseteq>\<^sub>p p"
  oops
theorem unrestricted_refines_corestricted_2: "is_feasible p \<Longrightarrow> p \<sqsubseteq>\<^sub>p p \<setminus>\<^sub>p C"
  oops

theorem corestrict_subprog: "p \<setminus>\<^sub>p C \<subseteq>\<^sub>p p" \<comment> \<open>Corestrict_subprog\<close>
  by (auto simp: specialize_def extends_def weakens_def restr_post_def corestrict_p_def corestrict_r_def restrict_r_def S_def Field_def strengthens_def)
...
theorem corestriction_prop: "D \<subseteq> C \<Longrightarrow> p \<setminus>\<^sub>p D \<sqsubseteq>\<^sub>p p \<setminus>\<^sub>p C" \<comment> \<open>T32\<close>
  oops
theorem corestriction_prop: "D \<subseteq> C \<Longrightarrow> p \<setminus>\<^sub>p C \<sqsubseteq>\<^sub>p p \<setminus>\<^sub>p D" \<comment> \<open>T32\<close>
  oops
...
theorem corestrict_subprogorder1: "D \<subseteq> C \<Longrightarrow> (p \<setminus>\<^sub>p D) \<subseteq>\<^sub>p (p \<setminus>\<^sub>p C)" \<comment> \<open>Corestrict_subprogorder1\<close>
  by (auto simp: corestrict_p_def specialize_def extends_def corestrict_r_def S_def Field_def weakens_def restr_post_def restrict_r_def strengthens_def)
...
theorem corestriction_subsafety: "q \<subseteq>\<^sub>p p \<Longrightarrow> q \<setminus>\<^sub>p C \<subseteq>\<^sub>p p \<setminus>\<^sub>p C" \<comment> \<open>Corestriction_subsafety\<close>
  by (auto simp: specialize_def extends_def weakens_def strengthens_def restrict_r_def corestrict_p_def corestrict_r_def S_def Field_def Range_iff Domain_iff Un_def)

theorem refinement_safety_corestriction: "q \<sqsubseteq>\<^sub>p p \<Longrightarrow> q \<setminus>\<^sub>p C \<sqsubseteq>\<^sub>p p \<setminus>\<^sub>p C" \<comment> \<open>Corestriction_refsafety\<close>
  oops

theorem implements_safety_corestriction: "implements a b \<Longrightarrow> implements (a \<setminus>\<^sub>p C) (b \<setminus>\<^sub>p C)"
  oops

theorem weakens_corestriction_1: "all_feasible [p, q] \<Longrightarrow> q \<sqsubseteq>\<^sub>p p \<Longrightarrow> weakens (q \<setminus>\<^sub>p C) (p \<setminus>\<^sub>p C)" \<comment> \<open>Corestriction_refsafety\<close>
  oops
theorem weakens_corestriction_2: "q \<sqsubseteq>\<^sub>p p \<Longrightarrow> weakens (p \<setminus>\<^sub>p C) (q \<setminus>\<^sub>p C)" \<comment> \<open>Corestriction_refsafety\<close>
  oops
theorem strengthens_corestriction_1: "q \<sqsubseteq>\<^sub>p p \<Longrightarrow> strengthens (p \<setminus>\<^sub>p C) (q \<setminus>\<^sub>p C)" \<comment> \<open>Corestriction_refsafety\<close>
  oops
theorem strengthens_corestriction_2: "q \<sqsubseteq>\<^sub>p p \<Longrightarrow> strengthens (q \<setminus>\<^sub>p C) (p \<setminus>\<^sub>p C)" \<comment> \<open>Corestriction_refsafety\<close>
