...
lemma refines_is_antisymetric: "p\<^sub>1 \<subseteq>\<^sub>p p\<^sub>2 \<Longrightarrow> p\<^sub>2 \<subseteq>\<^sub>p p\<^sub>1 \<Longrightarrow> p\<^sub>1 \<equiv>\<^sub>p p\<^sub>2"
  by (auto simp: refines_def extends_def weakens_def strengthens_def restrict_r_def equiv_def restr_post_def)

theorem refines_is_preorder: "p\<^sub>1 \<subseteq>\<^sub>p p\<^sub>1 \<and> (p\<^sub>2 \<subseteq>\<^sub>p p\<^sub>3 \<and> p\<^sub>3 \<subseteq>\<^sub>p p\<^sub>4 \<longrightarrow> p\<^sub>2 \<subseteq>\<^sub>p p\<^sub>4)"
  apply (rule conjI)
