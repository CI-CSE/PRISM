...
theorem property_on_ite_1: "ITE C p\<^sub>1 p\<^sub>2 = ITE (-C) p\<^sub>2 p\<^sub>1" \<comment> \<open>T52, T57\<close>
...
theorem refinement_safety_ite: "q\<^sub>1 \<subseteq>\<^sub>p p\<^sub>1 \<Longrightarrow> q\<^sub>2 \<subseteq>\<^sub>p p\<^sub>2 \<Longrightarrow> ITE C q\<^sub>1 q\<^sub>2 \<subseteq>\<^sub>p ITE C p\<^sub>1 p\<^sub>2" \<comment> \<open>T51\<close>
