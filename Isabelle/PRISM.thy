...
definition B':: "nat rel" where "B' = {(1,1)}"
definition mid':: "nat set" where "mid' = {}"
value "\<lparr>State={}, Pre=A', post={(x,y). x \<in> Domain B' \<and> y \<in> mid'}\<rparr> ; \<lparr>State={}, Pre=mid', post={(x,y). x \<in> mid' \<and> y \<in> Range B'}\<rparr>"
value "\<lparr>State={}, Pre=A', post=B'\<rparr>"

theorem law_3_3: "\<lparr>State=mid, Pre=A, post=B\<rparr> \<sqsubseteq>\<^sub>p \<lparr>State={}, Pre=A, post={(x,y). x \<in> Domain B \<and> y \<in> mid}\<rparr> ; \<lparr>State={}, Pre=mid, post={(x,y). x \<in> mid \<and> y \<in> Range B}\<rparr>"
